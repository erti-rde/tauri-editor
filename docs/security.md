...
```
 ...
 BibTeX/RIS/CSL (web)     ──parse───▶  import (M1)          fs plugin ──▶   $HOME read, write-all (incl. remove)
...
and `read_pdf_file` with no scope at all); write, rename or delete within the fs scope; and
...
| T3  | T / D  | After T1: delete or overwrite files under `$HOME` via the fs plugin                                        | None                                                                                        | `fs:write-all` includes `remove`; the scope is `$HOME` recursive                                      | **High** |
...
2. **Narrow the fs scope (T3)** — M1a. Replace `fs:write-all` + `fs:allow-home-read-recursive`
   ...
   directory through `FsExt::fs_scope()`. **Verify first** how the current scope actually
   resolves, by attempting a write outside the project from the dev console, and record the
   result here.
