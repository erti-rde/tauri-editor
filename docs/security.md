...
| T2  | E / I  | After T1: read any file via `read_pdf_file(path)` / `hash_file` / `read_directory`                         | Rust path commands accept only the open project, dialog picks and recorded locations (M1a-3)                                                                           | The fs plugin's `fs:allow-home-read-recursive` still reads all of `$HOME` from the webview (M1a-4)                                            | **High** |
