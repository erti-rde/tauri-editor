...
| --- | ------ | ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | -------- |
| T1  | E      | Script injection via a shared manuscript (citation HTML, new envelope `sources`)                           | DOMPurify allowlist on citation HTML (`sanitize.ts`); shape guards for envelope and CSL (M1b-10)                                                     | The guards reach manuscripts with format 1 (M1a-8); until then only lookups and sidecars pass through them | **High** |
| T2  | E / I  | After T1: read any file via `read_pdf_file(path)` / `hash_file` / `read_directory`                         | Rust path commands and the fs plugin reach only the open project, dialog picks and recorded locations (M1a-3, M1a-4)                                 | A project opened earlier in the session stays reachable until restart (Tauri's scope can't shrink)         | Medium   |
| T3  | T / D  | After T1: delete or overwrite files via the fs plugin                                                      | No `remove`, `rename` or `write-all` (M1a-4); writes only where the fs scope reaches: the project, dialog picks, app folders                         | A script can still overwrite files inside the open project; version history (M5-1) is the recovery         | Medium   |
...
| T8  | D      | Decompression/size bombs: huge PDFs, a 1M-entry `.bib`, a 50 MB manuscript, a deeply nested sidecar        | Ingest off the main thread; guard limits on entries, names, depth, strings and file size (M1b-10)                                                    | PDF size isn't checked; limits are provisional until M6-2                                                  | Medium   |
...
1. **Scope the Rust path commands (T2)** — done in M1a-3 (`src-tauri/src/scope.rs`). It also
   ...
   refused.
2. **Narrow the fs scope (T3)** — done in M1a-4. The webview has the six fs calls the app
   makes and no scope of its own: opening a project grants its folder, the dialog grants what the
   user picks, and resources and app folders stay readable. No `remove`; the trash command comes
   with M5-7, the first delete. Verified with an in-app probe (reads and writes outside the
   project refused, inside allowed, `remove` refused everywhere).
3. **Validate every untrusted format at the boundary (T1, T8)** — the guards are `src/lib/guard`
   (M1b-10), applied to lookup replies and sidecars now, to manuscripts in M1a-8 and to imports
   in M1b-9. Planned as: M1a (manuscript format), M1b (imports, sidecars), M5. One shape-guard module
