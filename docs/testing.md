...
| **App-in-browser** (new) | Playwright + `@tauri-apps/api/mocks` `mockIPC` with an in-memory fake backend          | every PR                      | full UI flows against a fake backend; boots `+page.svelte`, which nothing does today. **Spike first:** the mock must be installed before any module calls `invoke`, and the fake has to answer the fs, store and dialog plugins too. Size it before committing to it |
...
- Add the app-in-browser job (Playwright, Chromium) to `ci.yaml`.
