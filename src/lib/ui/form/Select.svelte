...
<Select.Root bind:value={value as never} {...restProps}>
	<Select.Trigger
		class="border-line-strong bg-surface-raised text-ink hover:border-line-strong focus-visible:border-accent flex w-full items-center justify-between rounded-md border px-3 py-2 text-sm shadow-sm disabled:cursor-not-allowed disabled:opacity-50"
	>
		<span class="truncate">
			{#if selectedLabel}
				{selectedLabel}
